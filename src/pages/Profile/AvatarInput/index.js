...
export default function AvatarInput() {
    ...
    // se defaultValue - recupera o seu url do avatar
    const [preview, setPreview ] = useState(defaultValue && defaultValue.url);

    // ref - guarda referencia do input, que a usa
    const ref = useRef();

    // ao iniciar a app, caso exista registra dados no state do o unForm, via registerField
    useEffect(() => {
        if(ref.current){
            registerField({
                name: 'avatar_id',
    ...
    async function handleChange(e) {
           // criando objeto para uso com multer partFormData (imagens)
